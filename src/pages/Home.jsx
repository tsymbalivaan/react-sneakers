...
export default function Home({ items, searchVal, setSearchVal, onFav, onAddToCart, isLoading }) {
	const renderItem = () => {
		return (
			isLoading ? [...Array(8)] : items.filter(item => item.title.toLowerCase().includes(searchVal.toLowerCase()))
		).map((item, index) => (
	...
	return (
		<div className="content p-40">
			...
			<div className="sneakers flex-wrap d-flex ">{renderItem()}</div>
		</div>
	);
}
